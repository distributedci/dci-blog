...
You got the idea :), now, let's say the awesome repo is not hosted in GitHub :-o, don't panic! Let's say that awesome repo is using Gerrit for collaboration, where do I start? glad you ask, next, we will take a look at a guided example on how the flow described above will apply in Gerrit.
...
We learned that using Gerrit to integrate changes is not that different from GitHub, at the end those are just tools or services that give a structure to how those changes are reviewed, validated, tested and merged.

It is important to highlight that this post is not about what tool or service is better, it is about not being afraid to collaborate using different tools or services. Sometimes it is frustrating having to deal with things done differently but at the end it is all about options. In my opinion it is better to deal with those differences than not having an option.

I hope this post helps to make your life easier and to motivate you to contribute to the next awesome project you find. Either if that project happens to be under Gerrit or something else.
