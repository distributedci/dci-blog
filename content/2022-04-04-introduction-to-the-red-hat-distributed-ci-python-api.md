...
Summary: In a previous post, you have been introduced to [Red Hat Distributed CI (DCI) infrastructure](introduction-to-the-red-hat-distributed-ci.html) and how it enables Red Hat partners to integrate into Red Hat CI workflow. Now, we will be focusing on how to interact with DCI Python API.
...
![screen capture showing where to click to get your credentials](images//home/hguemar/dist-git/dci/dci-blog/content/images/blog4_credentials-dci-retrieval.png)
...
```python
response = topic.list(c, where="name:RHEL-8.6")
...
response = topic.list_components(c, '3b8d7315-a941-467f-95ff-b789230bd50b')
content = json.loads(response.content)
```
...
- data: contains some relevant information specific to the product. e.g: for RHEL, hardware arches, channel, major, mijor version etc.
...
```python
response = job.create(c, 'a35675ca-e749-4e89-9b86-61191fc9fed4', components=["bc3766e0-35e9-4ab9-9e7a-b309c9e85270"])
...
```python
response = job.delete(c, 'f0956ebf-5bdb-47b2-96c0-0f07a6ed4e02', 'a86cb94d3e7eeaf1996c67972a2427d7')
