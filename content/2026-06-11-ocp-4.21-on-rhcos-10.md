...
As this OCP version approaches its release date, OCP releases 4.20 and 4.21 are announed to provide dual support for RHEL 9 and 10 RHCOS versions.

However, the different installation methods don't provide a explicit option to enable the deployment of the cluster on an operating system version different than the default one.

While previous versions of OCP offered the parameter "clusterOSImage" in the install-config.yaml file definition, newer releases seem to have deprecated this option in favor of a machine configuration specifying the OS image by means of specific OpenShift release payload images, as shown in the following example:
...
Likewise, although in this document we'll focus on the Agent Based Installer (ABI) deployment method, other methods like IPI can be used adapting the steps as required.
...
By the time of writting this article, the latest OCP 4.21 version available was 4.21.17.

We can  get the appropriate image URL using the *ocp adm* subcommand:
...
In order to do so, after creating the install-config.yaml file, we must create the subdirectory */manifests* in the same folder containing the install-config.yaml and place the machine configuration manifest there:
...
This gives us a good opportunity to test in advance the OCP integration with RHCOS 10 and, with the help of DCI, we may do so with all the advantages of a fully operative CI solution.