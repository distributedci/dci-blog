...
Imagine you have a [DCI queue]((dci-queue.html)) system with multiple resources, where each resource covers a set of nodes that can be used for a MNO deployment; for example, with 3 master nodes and 4 worker nodes, using installers such as IPI or ABI.
