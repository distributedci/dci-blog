...
* **Enhanced Security:** IPv6 incorporates IPsec (Internet Protocol Security) as a mandatory component, offering end-to-end encryption and authentication, which can be leveraged for more secure communication within the cluster.
...
An example of a node network definition with static IPv6 assignment in an OpenShift manifest will look like this:
...
The main idea is to define NodeNetworkConfigurationPolicy (NNCP) manifests that describe the desired network state for your nodes. These policies are interpreted by NMState to produce NetworkManager configurations. For more information about NMState and the syntax to use it, please visit NMState official website.
...
This is an example of an NNCP for a control plane node, assigning a static IPv6 address to a bonded interface named `bond0`. Here, we can observe the following:
    ...
    a. The `interfaces` variable is under `all.vars.nodes.vars.network_config`. This means that the configuration will apply to all nodes. If required to be different, each node can have its own definition in the respective inventory section.

    b. In this example, we are using a customized template under `all.vars.nodes.vars.network_config.template`. If not specified when using DCI OpenShift Agent, by default, it will use a template provided in the Red Hat CI OCP collection roles.

    c. This configuration is for a `bond0` interface in LACP mode with two physical ports. The `bond0` interface has IPv4 and IPv6 defined statically.

    d. Still under `network_config`, we can find `dns_server_ips` and `routes`, which can take the respective values from other variables, or be explicitly defined here.
